"""
SNAPSHOT BOOKMARK: PolygonWS websocket scaffolding (kept intentionally)

This file is a preserved snapshot of the Polygon websocket implementation used
for scalping workflows. It duplicates functionality that the production code
uses from `src.datafeeds.polygon_ws`.

Notes
- Not imported by production paths; kept for reference and experimentation.
- Prefer `from src.datafeeds.polygon_ws import PolygonWS` in live code.
- Environment: requires POLYGON_API_KEY and websocket-client installed.

If you update this snapshot, consider tagging a new lock in git to mark the
version used in any experiments.
"""
