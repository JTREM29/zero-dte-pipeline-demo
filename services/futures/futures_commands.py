...
def register_futures_commands(tree: app_commands.CommandTree, store: FuturesStore) -> None:
    def _ingest_health_snapshot() -> tuple[str, str, int | None, int | None]:
        """Return (state, reason, hb_age_s, scores_age_s) using the canonical classifier."""
        ...
        return ingest_state, ingest_reason, hb_age_s, scores_age_s
    ...
    async def futures(interaction: discord.Interaction) -> None:
        ...
        ingest_state, ingest_reason, hb_age_s, scores_age_s = await asyncio.to_thread(_ingest_health_snapshot)
        if ingest_state != "OK":
