# Schedules the polygon ingest loop to run automatically during U.S. market hours.
...
$logPath = Join-Path $repoRoot "logs\polygon_ingest.log"

if (-not (Test-Path $ingestScript)) {
    throw "Missing ingest launcher: $ingestScript"
}

$taskName = "TNT-Polygon-Ingest"
...
Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger -Principal $principal -Settings $settings -Description "Start polygon ingest loop during RTH" -Force
