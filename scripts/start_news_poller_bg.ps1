...
$out = Join-Path $logDirAbs 'news_poller_stdout.log'
$err = Join-Path $logDirAbs 'news_poller_stderr.log'
...
$spArgs = @{
    FilePath               = $py
    ArgumentList           = @('-u', $scriptPath, '--market')
...
Write-Output "[OPS] starting news_poller (--market)"
