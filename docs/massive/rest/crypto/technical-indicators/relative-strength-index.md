...
**Endpoint:** `GET /v1/indicators/rsi/{cryptoTicker}`
...
| --- | --- | --- | --- |
| `cryptoTicker` | string | Yes | The ticker symbol for which to get relative strength index (RSI) data. |
...
| `window` | integer | No | The window size used to calculate the relative strength index (RSI). i.e. a window size of 10 with daily aggregates would result in a 10 day moving average. |
